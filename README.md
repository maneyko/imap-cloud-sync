...
./main.py                              # every account in /etc/email-exporter/secrets
...
sudo mkdir -p /etc/email-exporter/secrets
cat | sudo tee /etc/email-exporter/secrets/me@example.com.toml <<'EOF'
[imap]
server   = "imap.example.com"
username = "me@example.com"
password = "..."
...
The repo that owns the machine holds no email-exporter logic beyond that call —
